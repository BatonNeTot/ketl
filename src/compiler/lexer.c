...
static bool ketl_lexer_parse_id(ketl_lexer_t* p_lexer, char next_symbol) {
    ...
    switch (first_symbol) {
        ...
        case 'c': {
            ...
            if (ketl_str_is_equal_n("cexport", p_lexer->p_source + p_lexer->offset, id_length)) {
                ketl_lexer_add_token(p_lexer, KETL_TOKEN_TYPE_CEXPORT, id_length);
                return true;
            }
        ...
        case 'e': {
            ...
            if (ketl_str_is_equal_n("extend", p_lexer->p_source + p_lexer->offset, id_length)) {
                ketl_lexer_add_token(p_lexer, KETL_TOKEN_TYPE_EXTEND, id_length);
                return true;
            }
            break;
        }
