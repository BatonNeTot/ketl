...
void ketl_gc_init(ketl_gc* p_gc, const ketl_allocator *p_allocator) {
    *p_gc = (ketl_gc){0};
    ...
    objects_init(&p_gc->v_root_objects, 16, p_allocator);
    objects_init(&p_gc->v_collect_buffer, 16, p_allocator);
    object_info_map_init(&p_gc->m_object_info, 16, p_allocator);
    p_gc->flag_usage = false;
}

void ketl_gc_deinit(ketl_gc* p_gc) {
    object_info_map_deinit(&p_gc->m_object_info);
    objects_deinit(&p_gc->v_collect_buffer);
    objects_deinit(&p_gc->v_root_objects);
}
...
void ketl_gc_reg(ketl_gc* p_gc, void* p_object, ketl_type* p_type, uint8_t flags) {
    ...
    object_info_map_get_or_insert_ref(&p_gc->m_object_info, p_object, &info);
    if (flags & KETL_GC_ROOT) {
        objects_push_back_copy(&p_gc->v_root_objects, p_object);
    }
}

static void* find_object_start(ketl_gc* p_gc, const void* p_inside_object, ketl_gc_info** pp_info) {
    if (p_gc->m_object_info.size == 0) {
        return NULL;
    }

    object_info_map_node* p_nodes = p_gc->m_object_info.p_nodes;
    object_info_map_node* p_node = p_nodes + p_gc->m_object_info.root_offset;
    ANN_FOREVER {
...
static void mark_objects(ketl_gc* p_gc) {
    ...
    objects_resize(&p_gc->v_collect_buffer, p_gc->v_root_objects.size);
    ketl_memcpy(p_gc->v_collect_buffer.p_data, p_gc->v_root_objects.p_data, p_gc->v_root_objects.size * sizeof(void*));
    while (p_gc->v_collect_buffer.size) {
        void* p_object = p_gc->v_root_objects.p_data[--p_gc->v_collect_buffer.size];
        ketl_gc_info* p_info;
        void* p_root = find_object_start(p_gc, p_object, &p_info);
        if (p_root && p_info->flag_usage != flag_usage) {
            p_info->flag_usage = flag_usage;
            objects_push_back_copy(&p_gc->v_root_objects, p_info->p_type);
            // TODO collect fields to v_collect_buffer
        }
    }
}
...
static void swipe_objects(ketl_gc* p_gc) {
    objects_clear(&p_gc->v_collect_buffer);
    p_gc->m_object_info.root_offset = visit_node_and_swipe(p_gc, p_gc->m_object_info.root_offset);
}
