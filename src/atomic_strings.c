...
void ketl_atomic_strings_init(ketl_atomic_strings* p_atomic_strings, const ketl_allocator* p_allocator) {
    ketl_atomic_strings_storage_init(&p_atomic_strings->v_storage, 16, p_allocator);
    ketl_atomic_strings_map_init(&p_atomic_strings->m_str_map, p_allocator);
}

void ketl_atomic_strings_deinit(ketl_atomic_strings* p_atomic_strings) {
    ketl_atomic_strings_map_deinit(&p_atomic_strings->m_str_map);
    ketl_atomic_strings_storage_deinit(&p_atomic_strings->v_storage);
}
...
ketl_atomic_string ketl_atomic_strings_get(ketl_atomic_strings* p_atomic_strings, const char* p_str, uint32_t length) {
    ...
    ketl_atomic_strings_storage* pv_symbols = &p_atomic_strings->v_storage;
    ketl_atomic_strings_map* pm_symbols_map = &p_atomic_strings->m_str_map;
    ...
    if (p_symbol_bucket->key == p_str) {
        char* p_check_data = pv_symbols->p_data;
        ketl_atomic_strings_storage_reserve(pv_symbols, pv_symbols->size + length + 1);
        if (p_check_data != pv_symbols->p_data) {
            p_check_data = pv_symbols->p_data;
            KETL_HASH_MAP_FOREACH(ketl_atomic_strings_map, pm_symbols_map, 
            __p_bucket->key = p_check_data + __p_bucket->value;);
        }

        const char* p_atomic_symbol = ketl_atomic_strings_storage_push_back_ref_n(pv_symbols, p_str, length);
        ketl_atomic_strings_storage_push_back_copy(pv_symbols, '\0');

        ANN_ASSERT(p_check_data == pv_symbols->p_data);

        p_symbol_bucket->key = p_atomic_symbol;
        p_symbol_bucket->value = (ketl_atomic_string)(p_atomic_symbol - pv_symbols->p_data);
    }
...
const char* ketl_atomic_strings_get_pointer(ketl_atomic_strings* p_atomic_strings, ketl_atomic_string a_str) {
    return KETL_ATOMIC_STRING_GET_POINTER(p_atomic_strings->v_storage.p_data, a_str);
}
